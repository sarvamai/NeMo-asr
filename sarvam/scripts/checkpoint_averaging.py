...
def main():
    ...
    parser.add_argument(
        '--top_k', type=int, default=5, help='If many checkpoints are logged, pick only top-k checkpoints for averaging (default=5)',
    )
    ...
    for model_fname_i, model_fname in enumerate(args.model_fname_list):
        ...
        # load state dicts
        # Sort checkpoint paths by lowest wer to select top-k
        checkpoint_paths = sorted(checkpoint_paths)
        # Select top k checkpoints
        checkpoint_paths = checkpoint_paths[:args.top_k]
